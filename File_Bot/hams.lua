...
local function keko_speed(data)
    ...
    local var = false
    local creator = database:sismember('speed:'..bot_id..'creator:'..chat_id, user_id) 
    local admin = database:sismember('speed:'..bot_id..'admins:', user_id)
    if creator then var = true end
    if admin then var = true end
    for k,v in pairs(sudo_users) do
    if user_id == v then var = true end end
    local keko_add_sudo = redis:get('speed:'..bot_id..'sudoo'..user_id..'')
    ...
    dl_cb, nil)
    database:set("speed:get:hms:msg:"..bot_id..msg.chat_id_..msg.id_..t2.id_,keko_ts[1])
    ...
    resolve_username(keko_ts[2],cb_speed)
    end
    end
    ...
    kekol = database:get("speed:get:hms:msg:"..bot_id..keko_ts[1]..keko_ts[2])
    if kekol then 
    ...
    database:set("speed:get:hms:gr:"..bot_id..msg.chat_id_,"speed")
    send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعطيل اهمس", 1, 'html')
    end 
    if text and text == "تفعيل اهمس" then 
    database:del("speed:get:hms:gr:"..bot_id..msg.chat_id_)
